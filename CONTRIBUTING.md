...
3. Keep portable behavior independent of Codex, Claude Code, Kimi Code and OpenCode. Harness-specific behavior belongs in a thin adapter. A newly evaluated harness is not supported until Setup, Doctor, schema, adapter and live behavior coverage are present.
...
Keep each pull request to one reviewable change. Explain the capability, maturity
class, evidence, provenance classification and any harness-specific limitations. A
portable contract is not the same claim as runtime integration, and installation or
discovery is not the same claim as live behavior. A contribution is not ready while
tests fail or its provenance record is missing.
